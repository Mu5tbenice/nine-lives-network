...
async function getTodayRaid() {
  ...
  console.log(`[NarrativeEngine] Created raid for ${today}: "${narrative.title}"`);
  return newRaid;
}
...
async function scrapeAndCount(tweetId, currentStandings = {}) {
  ...
  try {
    ...
    console.log(`[NarrativeEngine] Scraped ${replies.length} replies, ${newRepliers.length} new unique raiders`);
    return standings;
...
async function awardRaidPoints(playerId, points) {
  try {
    ...
    console.error(`[NarrativeEngine] Points error for player ${playerId}:`, err.message);
  }
}
...
function getSortedCommunities(standings) {
  return Object.entries(standings)
    .map(([tag, data]) => ({ tag, unique: data.unique, users: data.users, player_ids: data.player_ids }))
...
async function postOpening() {
  ...
  try {
    ...
    if (!narrative) {
      console.error('[NarrativeEngine] Narrative not found:', raid.narrative_id);
      return;
    }
...
async function postMidDay() {
  ...
  try {
    ...
    const standings = await scrapeAndCount(raid.tweet_1_id, raid.standings || {});
...
async function postResolution() {
  ...
  try {
    ...
    console.log(`[NarrativeEngine] Resolution posted. Winner: ${winnerCommunity} (${winnerCount} raiders)`);
...
async function periodicScrape() {
  try {
    ...
    const total = Object.values(standings).reduce((sum, c) => sum + c.unique, 0);
    console.log(`[NarrativeEngine] Periodic scrape: ${total} total unique raiders`);
