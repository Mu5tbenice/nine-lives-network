...
- [ ] 4.0 Nightly zone-identity cron fix + diagnostic (§9.19 → Phase 1 critical) **(unblocked 2026-04-20 by Task 4.5 decisions)** (est: M — re-estimated from L after Task 4.5 locked the design)
  ...
  - [x] 4.0.3 ~~Refactor combatEngine zone-bonus reader to read `zones` table instead of `zone_control` per Q4.~~ **Landed 2026-04-20 in PR #?.** `refreshZoneBonusCache` at `server/services/combatEngine.js:124-142` now reads from `zones`. Zones-list merge at `server/routes/zones.js:829-841` also flipped to prefer `zones` over `zone_control` so frontend and combat see the same authoritative value. `zone_control.dominant_house` column drop deferred to PR #145. Closes §9.21.
