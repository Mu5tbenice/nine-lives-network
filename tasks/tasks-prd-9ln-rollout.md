...
- [ ] 4.0 Nightly zone-identity cron fix + diagnostic (§9.19 → Phase 1 critical) **(unblocked 2026-04-20 by Task 4.5 decisions)** (est: M — re-estimated from L after Task 4.5 locked the design)
  ...
  - [x] 4.0.5 ~~Delete orphaned `/midnight-reset` endpoint per §9.20.~~ **Landed 2026-04-20 in PR #?.** Endpoint at `server/routes/zones.js` deleted. Zero consumers confirmed via grep before deletion. Closes §9.20.
