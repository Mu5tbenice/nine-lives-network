...
- [ ] 17.0 Auto-rejoin UX redesign + session timeout semantic refactor (est: M-L, 3-5 PRs) **Status: backlog. Scope expanded 2026-04-20 to include §9.41 refactor — the UX work and the code refactor are tightly coupled and need to ship together.**
  ...
  **Sequencing plan (3-5 PRs).**
  1. **`daily_combat_stats` infrastructure.** New server-side tracking source with 00:00 UTC reset. Client sidebar SESSION view reads from this instead of the page-scoped `S.combatMetrics`. Decision TBD: Supabase table vs in-memory with periodic flush. No UX changes yet — purely adding the data source.
  2. **Remove `SESSION_MS` + server-side inactivity timeout.** Delete the 2h constant, remove the `arena:session_expired` server broadcast trigger on the 2h path. Client handler (from PR #158) stays dormant for one deploy cycle as backward-compat, then removed. Manual deploys now persist until KO or explicit withdraw.
  3. **Sidebar rewrite.** Wire SESSION toggle to the new daily source. Reconsider DAY/SESSION/ROUND sort-label clarity — possibly DAY / LIFETIME / THIS-ROUND. Depends on #1.
  4. **Auto-rejoin UX (original Task 17.0 scope).** Flip `S._autoRedeploy` default to `true` at init. Rework `_showRejoinPrompt` copy + buttons: primary action becomes "CHANGE BUILD" (opens deploy modal), secondary becomes "WAIT" (auto-rejoin proceeds silently). Deploy modal auto-rejoin toggle stays, marked as "keep auto-rejoining", default-checked. 1h cap messaging throughout. Can parallel-ship with #2.
