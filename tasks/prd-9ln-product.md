...
### 9.79 Arena bottom tray graphics disappear mid-session despite player still deployed
...
**Resolved 2026-04-26 in PR #290** — full handshake live end-to-end. Bot username sourced at runtime via `getBotUsername()` (exported from `nerm-telegram.js` after `bot.getMe()` resolves) with env (`TELEGRAM_BOT_USERNAME`) + literal `Nerm9LV_Bot` fallbacks. Community URL defaults to `https://t.me/iduj9q8mx98`, overridable via `TELEGRAM_GROUP_URL` env var. Bundled with §9.103 + §9.104.

**Follow-up 2026-04-26 in PR #291** — initial UX shipped a t.me/<bot>?start=<code> deeplink to a private chat with @Nerm9LV_Bot, which on smoke produced no bot response (Wray: *"the link takes me to nerm bot but the start does nothing, usually have to start him in the server its not a DM thing"*). The bot's existing `bot.on('message')` handler returns early on non-group chats and was never built for DMs. Pivoted to a **group-canonical** flow: `/settings` LINK TELEGRAM now reveals a code panel with "JOIN 9LV TELEGRAM" + a copyable `/start <code>` command; the user joins the group, pastes the command, the bot processes in-channel. Drives community traffic by design — group members see the bot reply, which seeds activity. Bot's `/start <code>` handler unchanged (already accepted the payload from any chat type); only the FE flow + bot success-reply copy changed. Captured in `feedback_nerm_group_canonical.md`.

**Follow-up 2026-04-26 in PR #?** — `/start <code>` triggered every other bot in the 9LV group (Wray: *"/start is going to trigger other bots so we need to do something different"*). `/start` is the standard onboarding command, registered globally by most bots. Switched to **`/link@Nerm9LV_Bot <code>`** as the canonical paste form: `/link` is a non-standard command few bots register, and the `@Nerm9LV_Bot` suffix scopes the command to Nerm even when collisions exist. Bot now registers both `/link <code>` (preferred) and `/start <code>` (legacy / external-deeplink fallback) — both delegate to the same `handleLinkStart` helper. `/link` without a payload returns a usage hint. FE pulls the bot username from the API response so the copy adapts if the bot ever rebrands.
