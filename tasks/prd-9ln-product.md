...
**Fully resolved 2026-04-20 in PR #?.** End-to-end zone-identity pipeline for V4 is now live and correct. Resolution chain: PR #141 (diagnostic reframe), PR #143 (round-end writer + recalc aggregation per Q1/Q2), PR #144 (combat reader switch + merge flip + guild-tag UI per Q3/Q4), PR #145a (code cleanup of deprecated column reads), PR #145b (schema drop via `supabase/migrations/20260419144153_drop_zone_control_deprecated_columns.sql`). Combat, recalc, and UI all read from `zones` as the single source of truth; deprecated V1 columns are gone.
